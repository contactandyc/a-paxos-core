...
MACRO_TEST(paxos_rejects_unsupported_config_changes) {
    ...
    // Core should instantly drop this because reconfig is disabled
...
MACRO_TEST(paxos_rejects_malformed_proposals) {
    ...
    // data_len > 0 but data pointer is NULL
    ...
    // Firewall must reject it, keeping log at slot 1
...
MACRO_TEST(paxos_leader_steps_down_on_higher_prepare) {
    ...
    // Node 2 starts an election with a higher epoch
    ...
    // Node 1 must instantly revert to learner state
    MACRO_ASSERT_EQ_INT(paxos_state(p), PAXOS_STATE_LEARNER);

    // Node 1 must refuse local proposals now
...
MACRO_TEST(paxos_stale_follower_fetches_truth_instead_of_corrupting) {
    uint64_t peers[] = {1, 3};
    paxos_t* p = paxos_create(2, peers, 2); // We are node 2 (follower)

    // Node 2 accepts stale value "X" at ballot 5, slot 1
    paxos_log_accept(p, 1, 5, ENTRY_NORMAL, 0, 0, (uint8_t*)"X", 1);
    p->promised_ballot = 10; // New leader has ballot 10

    // Leader tells Node 2 that slot 1 is committed.
    paxos_msg_t commit = { .type = MSG_COMMIT_NOTICE, .to = 2, .from = 1, .ballot = 10, .commit_index = 1 };
    paxos_step_remote(p, &commit);

    // Because Node 2's ballot (5) != promised (10), it MUST NOT apply X!
    // It should request a fetch instead.
    MACRO_ASSERT_EQ_INT(p->local_commit_index, 0); // Did not commit!
