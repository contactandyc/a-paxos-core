...
void paxos_rebuild_config(paxos_t* p) {
    ...
    // Fast iteration through chunks instead of flat arrays
    for (size_t c = 0; c < p->log_chunks_cap; c++) {
        if (!p->log_chunks[c]) continue;
        for (size_t o = 0; o < PAXOS_LOG_CHUNK_SIZE; o++) {
            if (!p->log_chunks[c]->slots[o].has_value) continue;

            paxos_entry_t* e = &p->log_chunks[c]->slots[o].entry;
...
bool paxos_log_accept(paxos_t* p, uint64_t slot, uint64_t ballot, entry_type_t type, uint64_t cid, uint64_t cseq, const uint8_t* data, size_t data_len) {
    ...
    // Grow the chunk pointer array if necessary
    ...
    if (slot_data->has_value) {
        paxos_entry_destroy(&slot_data->entry);
    }
    ...
    slot_data->entry.data = paxos_payload_alloc(data, data_len); // Ref-Counted Allocation

    slot_data->has_value = true;
    slot_data->unstable = true;
    slot_data->chosen = false;

    if (type == ENTRY_CONF_ADD || type == ENTRY_CONF_REMOVE || type == ENTRY_CONF_JOINT || type == ENTRY_CONF_FINAL) {
    ...
    for (size_t c = 0; c < p->log_chunks_cap; c++) {
        ...
        for (size_t o = 0; o < PAXOS_LOG_CHUNK_SIZE; o++) {
            if (p->log_chunks[c]->slots[o].has_value && p->log_chunks[c]->slots[o].unstable) {
                if (!paxos_entry_clone(&unstable_arr[j++], &p->log_chunks[c]->slots[o].entry)) {
    ...
    for (size_t c = start_c; c < p->log_chunks_cap; c++) {
        ...
        for (size_t o = 0; o < PAXOS_LOG_CHUNK_SIZE; o++) {
            ...
            if (slot_data->has_value && slot_data->entry.slot >= start_slot) {
                if (!paxos_entry_clone(&suffix[j++], &slot_data->entry)) { p->fatal_error = true; return NULL; }
            }
        }
    }
    *out_count = count;
    ...
    for (size_t c = start_c; c <= end_c; c++) {
        ...
        for (size_t o = 0; o < PAXOS_LOG_CHUNK_SIZE; o++) {
            ...
            if (s_data->has_value && s_data->entry.slot >= start_slot && s_data->entry.slot <= end_slot) {
                if (!paxos_entry_clone(&range[j++], &s_data->entry)) { p->fatal_error = true; return NULL; }
            }
        }
    }
    *out_count = count;
...
void paxos_advance_local_commit(paxos_t* p) {
    while (p->local_commit_index < p->leader_commit_hint) {
        ...
        paxos_log_slot_t* slot_data = &p->log_chunks[c_idx]->slots[c_off];
        if (slot_data->entry.accepted_ballot == p->promised_ballot) slot_data->chosen = true;
...
void paxos_compact(paxos_t* p, uint64_t compact_slot) {
    ...
    // 1. Bake configs into the baseline mask
    ...
    // 2. Release payloads behind the horizon
    for (size_t c = 0; c <= end_c && c < p->log_chunks_cap; c++) {
        if (!p->log_chunks[c]) continue;
        for (size_t o = 0; o < PAXOS_LOG_CHUNK_SIZE; o++) {
            paxos_log_slot_t* s_data = &p->log_chunks[c]->slots[o];
    ...
    // FAANG: O(1) Chunk Wipeouts. Free fully obsolete chunks.
    for (size_t c = 0; c < end_c; c++) {
        if (p->log_chunks[c]) {
            free(p->log_chunks[c]);
            p->log_chunks[c] = NULL;
        }
