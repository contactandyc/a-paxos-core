...
MACRO_TEST(malformed_joint_config_lengths_are_rejected) {
    ...
    paxos_entry_t e = { .type = ENTRY_CONF_JOINT, .data = bad_data, .data_len = 10 };
    paxos_msg_t prop = { .type = MSG_PROPOSE, .entries = &e, .num_entries = 1 };

    paxos_step_local(p, &prop);

    // Engine must reject the malformed proposal entirely
...
MACRO_TEST(joint_config_with_duplicate_nodes_is_rejected) {
    ...
    paxos_entry_t e = { .type = ENTRY_CONF_JOINT, .data = (uint8_t*)bad_nodes, .data_len = sizeof(bad_nodes) };
    paxos_msg_t prop = { .type = MSG_PROPOSE, .entries = &e, .num_entries = 1 };
...
MACRO_TEST(final_config_with_nonzero_payload_is_rejected) {
    ...
    uint64_t garbage = 99;
    paxos_entry_t e = { .type = ENTRY_CONF_FINAL, .data = (uint8_t*)&garbage, .data_len = sizeof(uint64_t) };
    paxos_msg_t prop = { .type = MSG_PROPOSE, .entries = &e, .num_entries = 1 };
